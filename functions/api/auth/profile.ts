...
export const onRequestPatch: PagesFunction<Env> = async (context) => {
  ...
  let body: { displayName?: string; avatarIcon?: string | null; defaultEngineId?: string | null };
  try {
  ...
  const { displayName, avatarIcon, defaultEngineId } = body;
  ...
  const user = await env.DB.prepare(`
    SELECT id, email, display_name, avatar_url, is_admin, default_engine_id
    FROM profiles
    WHERE id = ?
